...
const BOOKMARKLET = `javascript:(function(){const dpr=window.devicePixelRatio;const zoomRatio=window.innerWidth/window.outerWidth;const warnings=[];if(Math.abs(dpr-1)>0.05&&Math.abs(dpr-2)>0.05){warnings.push(\`Unusual devicePixelRatio: \${dpr}\`);}if(Math.abs(zoomRatio-1)>0.1){warnings.push(\`Browser appears zoomed (ratio \${zoomRatio.toFixed(2)}). Set zoom to 100%.\`);}if(warnings.length>0){const proceed=confirm('Extraction warning:\\n'+warnings.join('\\n')+'\\n\\nComputed values may be inaccurate. Run on the real page at 100% zoom, NOT in DevTools device emulation.\\n\\nContinue anyway?');if(!proceed)return;}const TEXT_TAGS=new Set(['H1','H2','H3','H4','H5','H6','SPAN','P','LABEL','A','BUTTON']);const HARD_CEILING=12;const extract=(el,depth)=>{if(depth>HARD_CEILING)return null;const hasDirectText=Array.from(el.childNodes).some(n=>n.nodeType===3&&n.textContent.trim().length>0);const isTextish=hasDirectText||TEXT_TAGS.has(el.tagName);const shouldCapture=depth<=4||isTextish;const children=[];for(const child of el.children){const c=extract(child,depth+1);if(c)children.push(c);}if(!shouldCapture&&children.length===0)return null;const leafText=el.childElementCount===0?el.textContent?.trim().slice(0,80)||null:hasDirectText?Array.from(el.childNodes).filter(n=>n.nodeType===3).map(n=>n.textContent.trim()).join(' ').slice(0,80)||null:null;if(!shouldCapture){return{tag:el.tagName.toLowerCase(),id:el.id||null,classes:el.className||null,styles:{},rect:{x:0,y:0,w:0,h:0},textContent:null,children};}const cs=getComputedStyle(el);const props=['display','flexDirection','gap','padding','paddingTop','paddingRight','paddingBottom','paddingLeft','margin','marginTop','marginRight','marginBottom','marginLeft','width','height','backgroundColor','color','fontSize','fontFamily','fontWeight','lineHeight','letterSpacing','borderRadius','border','boxShadow','position','alignItems','justifyContent'];const styles={};props.forEach(p=>{styles[p]=cs[p];});const rect=el.getBoundingClientRect();return{tag:el.tagName.toLowerCase(),id:el.id||null,classes:el.className||null,styles,rect:{x:Math.round(rect.x),y:Math.round(rect.y),w:Math.round(rect.width),h:Math.round(rect.height)},textContent:leafText,children};};const data={url:location.href,viewport:{w:innerWidth,h:innerHeight},devicePixelRatio:dpr,innerWidth:window.innerWidth,outerWidth:window.outerWidth,screenWidth:window.screen.width,captureContext:{zoomRatio:parseFloat(zoomRatio.toFixed(2)),devicePixelRatio:dpr},tree:extract(document.body,0)};const json=JSON.stringify(data);navigator.clipboard.writeText(json).then(()=>alert('Computed styles copied to clipboard. Paste into the fidelity checker.'),()=>{const ta=document.createElement('textarea');ta.value=json;document.body.appendChild(ta);ta.select();document.execCommand('copy');document.body.removeChild(ta);alert('Copied! Paste into the fidelity checker.');});})();`
